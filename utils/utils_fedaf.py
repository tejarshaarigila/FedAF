...
from utils.networks import ConvNet  # Ensure this import is correct
...
def get_network(model, channel, num_classes, im_size):
    """
    Initializes the specified neural network model.

    Args:
        model (str): Model architecture name ('ConvNet').
        channel (int): Number of input channels.
        num_classes (int): Number of output classes.
        im_size (tuple): Image size (height, width).
    ...
    """
    torch.random.manual_seed(int(time.time() * 1000) % 100000)  # Seed for reproducibility
